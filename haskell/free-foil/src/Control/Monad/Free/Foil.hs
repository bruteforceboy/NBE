...
import Generics.Kind (Atom (Kon, (:@:)), Exists, Field, GenericK (..), Var0, Var1, (:$:), (:*:), (:+:))
...
deriving instance
  (forall x y. NFData (binder x y), forall scope term. (NFData scope, NFData term) => NFData (sig scope term)) =>
  NFData (AST binder sig n)

instance GenericK (ScopedAST binder sig) where
  type
    RepK (ScopedAST binder sig) =
      Exists
        Foil.S
...
instance
  (Bifunctor sig, Foil.CoSinkable binder, Foil.SinkableK binder) =>
  Foil.RelMonad Foil.Name (AST binder sig)
  where
  rreturn = Var
  ...
  (ScopedAST binder2 body2) =
    case Foil.unifyPatterns binder1 binder2 of
      ...
      Foil.SameNameBinders {} ->
        -- after seeing this we know that body scopes are the same
...
unsafeEqScopedAST ::
  ...
  Bool
unsafeEqScopedAST (ScopedAST binder1 body1) (ScopedAST binder2 body2) =
  and
