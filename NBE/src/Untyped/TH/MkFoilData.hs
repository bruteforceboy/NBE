...
module Untyped.TH.MkFoilData where

import Language.Haskell.TH
import Language.Haskell.TH.Syntax (addModFinalizer)
import Untyped.Foil qualified as Foil
import Untyped.TH.Util
...
mkFoilPattern nameT patternT = do
  ...
  where
    ...
    toPatternCon ::
      [TyVarBndr BndrVis] ->
      Name ->
      -- \^ Name for the starting scope type variable.
      Con ->
      -- \^ Raw pattern constructor.
      Q Con
    toPatternCon tvars n (NormalC conName params) = do
      ...
      where
        ...
        toPatternConParams ::
          Int ->
          -- \^ Index of the component in the constructor.
          Name ->
          -- \^ Current scope (after processing any previous bindings).
          [BangType] ->
          -- \^ Leftover pattern components.
          Q (Name, [BangType])
        -- \^ Resulting extended scope and a list of corresponding scope-safe components.
